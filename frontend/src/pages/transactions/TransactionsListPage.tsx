...
import { Button, Notification, Table, type Column } from '../../components/common'
  ...
  switch (transaction.type) {
    case 'DECLARACION_OPERADOR_INICIO_DIA':
      if (transaction.capitalDeclaration && transaction.capitalDeclaration.length > 0) {
        ...
        return `Declaración: ${decl.declaredBalance.toFixed(2)} ${
          decl.currency
        } (Fuente ID: ${decl.fundingSourceId.slice(-6)})`
      }
  ...
  const fetchTransactions = useCallback(async (page: number, limit: number) => {
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        const errorMessage = 'Error desconocido al cargar transacciones'
        setError(errorMessage)
        toast.error(errorMessage)
        logger.error('Error desconocido al cargar transacciones:', err)
      }
    }
    setIsLoading(false)
  }, [])
...
// Simple logger implementation with proper typing
const logger = {
