...
import type { Game, GameItem, CreateGameItemData } from '../../types/game.types'
import Card from '../../components/common/Card'
import Button from '../../components/common/Button'
import { LoadingSpinner, Notification, FilterBar } from '../../components/common'
import { GameItemTable, GameItemForm } from '../../components/games'
...
const GameItemsPage: React.FC = () => {
  ...
  const fetchGameAndItems = useCallback(async () => {
    ...
    try {
      ...
      if (err instanceof Error) {
        setError(err.message || 'Error al cargar información del juego')
        console.error('Error al cargar información del juego:', err)
      } else {
        setError('Error desconocido al cargar información del juego.')
        console.error('Error desconocido al cargar información del juego:', err)
      }
    } finally {
      setIsLoading(false)
    }
  ...
  const handleCreateGameItem = async (itemData: CreateGameItemData) => {
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        setError('Error desconocido al crear ítem.')
        console.error('Error desconocido al crear ítem:', err)
      }
    } finally {
  ...
  const handleUpdateItemStock = async (itemId: string, newStock: number) => {
    try {
      ...
      setGameItems((prevItems) =>
        prevItems.map((item) => (item._id === itemId ? { ...item, currentStock: newStock } : item)),
      )
      ...
      if (err instanceof Error) {
        ...
        setError('Error desconocido al actualizar stock.')
        console.error('Error desconocido al actualizar stock:', err)
      }
    }
  }
  ...
  const handleUpdateItemStatus = async (itemId: string, newStatus: 'active' | 'inactive' | 'archived') => {
    ...
    try {
      ...
      if (err instanceof Error) {
        setError(err.message || 'Error al actualizar el estado del ítem')
        console.error('Error al actualizar el estado del ítem:', err)
      } else {
        setError('Error desconocido al actualizar el estado del ítem.')
        console.error('Error desconocido al actualizar el estado del ítem:', err)
      }
    } finally {
      setIsLoading(false)
    }
  ...
  const handleDeleteGameItem = async (itemId: string) => {
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        setError('Error desconocido al eliminar el ítem.')
        console.error('Error desconocido al eliminar el ítem:', err)
      }
    } finally {
      setIsLoading(false)
    }
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Ítems de {game?.name}</h1>
            <div className="flex items-center mt-1">
              <p className="text-gray-600 dark:text-gray-400">Código: {game?.shortName}</p>
        ...
        {successMessage && <Notification type="success" message={successMessage} onClose={clearMessages} />}
