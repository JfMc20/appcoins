...
import { FundingSource, UpdateFundingSourceData } from '../../types/fundingSource.types';
...
const EditFundingSourcePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchFundingSource = async () => {
      ...
      try {
        ...
        const apiError = err.response?.data?.message || err.message || 'Error al cargar los datos de la fuente de fondos.';
  ...
  const handleSubmit = async (data: UpdateFundingSourceData) => {
    ...
    try {
      ...
      const apiError = err.response?.data?.message || err.message || 'Error al actualizar la fuente de fondos.';
      setErrorMessage(apiError);
      toast.error(apiError);
    } finally {
