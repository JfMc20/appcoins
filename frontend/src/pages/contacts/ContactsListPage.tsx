...
import { Contact, GetContactsParams, PaginatedContactsResponse } from '../../types/contact.types';
...
const ContactsListPage: React.FC = () => {
  ...
  const fetchContacts = useCallback(async (params: GetContactsParams) => {
    ...
    try {
      ...
      const apiError = err.response?.data?.message || err.message || 'Error al cargar los contactos';
      setError(apiError);
  ...
  const debouncedFetch = useCallback(debounce((params: GetContactsParams) => fetchContacts(params), 500), [fetchContacts]);
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      ...
      const apiError = err.response?.data?.message || 'Error al eliminar el contacto.';
      setError(apiError);
      toast.error(apiError);
    }
  };
    ...
    <DashboardLayout>
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          ...
          <Button 
            variant="primary"
            onClick={() => navigate(Pathnames.contacts.new)}
          >
            Nuevo Contacto
          </Button>
        </div>
        ...
        <div className="mb-4 p-4 bg-gray-100 dark:bg-gray-800 rounded-lg shadow">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <select
              ...
              className="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-gray-900 dark:text-white"
            >
              {CONTACT_TYPES.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
            </select>
             <select
              name="status"
              ...
              className="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-gray-900 dark:text-white"
            >
              {CONTACT_STATUSES.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
            </select>
          </div>
        </div>
        ...
        {!loading && !error && (
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-x-auto">
            {contacts.length > 0 ? (
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    ...
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Email P.</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Teléfono P.</th>
                  ...
                  {contacts.map((contact) => (
                    <tr key={contact._id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                       ...
                       <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{contact.primaryEmail || '-'}</td>
                       <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{contact.primaryPhone || '-'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500 dark:text-gray-300">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                          ...
                          contact.status === 'potential' ? 'bg-blue-100 text-blue-800 dark:bg-blue-800 dark:text-blue-100' :
              ...
              <p className="p-6 text-center text-gray-500 dark:text-gray-400">No se encontraron contactos con los filtros actuales.</p>
            )}
          </div>
        )}
        ...
        {!loading && !error && contacts.length > 0 && pagination.totalPages > 1 && (
            <div className="mt-6 flex justify-between items-center">
                <span className="text-sm text-gray-700 dark:text-gray-400">
                    Mostrando {((pagination.currentPage - 1) * pagination.itemsPerPage) + 1} - {Math.min(pagination.currentPage * pagination.itemsPerPage, pagination.totalItems)} de {pagination.totalItems} contactos
                </span>
